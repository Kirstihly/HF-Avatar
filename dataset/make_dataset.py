...
def post_process_invisible_joints(body):
    if len(body.shape) == 2:
        if body[4, 2] < 0.2:
            body[4, :2] = 3.0 / 4 * body[8, :2] + 1.0 / 4 * body[1, :2]
            body[4, 2] = 0.3
        if body[7, 2] < 0.2:
            body[7, :2] = 3.0 / 4 * body[8, :2] + 1.0 / 4 * body[1, :2]
            body[7, 2] = 0.3
...
def load_normalized_joints_from_openpose(file_path, resolution=(1080, 1080)):
    ...
    body = body * np.array([2.0 / resolution[1], -2.0 / resolution[0], 1.0]) + np.array(
        [-1.0, 1.0, 0.0]
    )
    body[:, 0] *= 1.0 * resolution[1] / resolution[0]

    face = face * np.array([2.0 / resolution[1], -2.0 / resolution[0], 1.0]) + np.array(
        [-1.0, 1.0, 0.0]
    )
    face[:, 0] *= 1.0 * resolution[1] / resolution[0]

    return body, face


def load_joints_from_mmpose(root_dir, name, length):
    body_joints = np.load(root_dir + "/2d_joints/" + name + "/body25_keypoints2d.npy")[
        :length
    ]
    face_joints = np.load(root_dir + "/2d_joints/" + name + "/face70_keypoints2d.npy")[
        :length
    ]
...
def load_normalized_joints_from_mmpose(root_dir, name, length, resolution=(1080, 1080)):
    ...
    body_joints = body_joints * np.array(
        [2.0 / resolution[1], -2.0 / resolution[0], 1.0]
    ) + np.array([-1.0, 1.0, 0.0])
    body_joints[:, :, 0] *= 1.0 * resolution[1] / resolution[0]

    face_joints = face_joints * np.array(
        [2.0 / resolution[1], -2.0 / resolution[0], 1.0]
    ) + np.array([-1.0, 1.0, 0.0])
    face_joints[:, :, 0] *= 1.0 * resolution[1] / resolution[0]
...
def distinguish_rotate_direction(root_dir, name, length):
    ...
    body_joints, face_joints = load_normalized_joints_from_mmpose(
        root_dir, name, length
    )
...
def get_all_silhouettes(root_dir, name, length=0):
    ...
    for idx in range(1, length + 1):
        silhouette_img = cv2.imread(
            root_dir + "/mask_mat/" + name + "/" + str(idx).zfill(PADDING) + ".png"
        )
...
def get_all_joints_from_mmpose(root_dir, name, length=0):
    ...
    body_joints, face_joints = load_normalized_joints_from_mmpose(
        root_dir, name, length, resolution=(1024, 1024)
    )
...
class apose_train_dataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        for i in range(0, self.view_num):
            ...
            mask = cv2.imread(self.mask_dir + f"{str(target_idx).zfill(PADDING)}.png")[
                :, :, 0
            ]
            ...
            body, face = load_joints_from_openpose(
                self.joints_dir
                + "json/"
                + str(target_idx).zfill(PADDING)
                + "_keypoints.json"
            )
            body = post_process_invisible_joints(body)

            pose_encoding = np.load(
                self.joints_dir + "encoding/" + str(target_idx).zfill(PADDING) + ".npz"
            )["arr_0"]
        ...
        pose = self.params["poses"][
            index : index + self.view_num * len(self.idx_list) : len(self.idx_list)
        ]
        trans = self.params["trans"][
            index : index + self.view_num * len(self.idx_list) : len(self.idx_list)
        ]
        shape = self.params["betas"][
            index : index + self.view_num * len(self.idx_list) : len(self.idx_list)
        ]
        shape = np.mean(shape, axis=0, keepdims=True)

        masks = numpy2tensor(
            np.array(masks) / 255.0
...
class apose_test_dataset(Dataset):
    def __init__(
        self, root_dir, name, length, idx_list, view_num, img_size=(1024, 1024)
    ):
        self.mask_dir = f"{root_dir}/mask_mat/{name}/"
    ...
    def __getitem__(self, item):
        ...
        for i in range(0, self.view_num):
            target_idx = self.idx_list[item] + i * len(self.idx_list)
            # mask_path = self.dir + 'frames_mat/' + self.name + '/' + str(target_idx).zfill(PADDING) + '.png'
            mask = cv2.imread(self.mask_dir + f"{str(target_idx).zfill(PADDING)}.png")[
                :, :, 0
            ]
            mask = cv2.resize(mask, self.img_size)
            ...
            # body = post_process_invisible_joints(body)
            _body, _face = load_joints_from_openpose(
                self.joints_dir
                + "json/"
                + str(target_idx).zfill(PADDING)
                + "_keypoints.json"
            ...
            pose_encoding = generate_pose_encoding(
                _body, target_idx, resolution=self.img_size[0]
            )
...
class diff_optimizer_dataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        for i in range(0, self.view_num):
            target_idx = self.idx_list[item] + i * len(self.idx_list)
            img_path = f"{self.root_dir}/frames_mat/{self.name}/{str( target_idx ).zfill(PADDING)}.png"
            ...
            silhouette_img_path = f"{self.root_dir}/mask_mat/{self.name}/{str( target_idx ).zfill(PADDING)}.png"
            silhouette_img = cv2.imread(silhouette_img_path)
            # silhouette_img[np.where(silhouette_img > 128)] = 255
            # silhouette_img[np.where(silhouette_img < 128)] = 0
            _, silhouette_img = cv2.threshold(
                silhouette_img, 128, 255, cv2.THRESH_BINARY
            )
            ...
            body, face = load_normalized_joints_from_openpose(
                f"{self.root_dir}/2d_joints/{self.name}/json/{str( target_idx ).zfill(PADDING)}_keypoints.json",
                resolution=self.img_size,
            )
            # body, face = self.body_keypoints[target_idx - 1], self.face_keypoints[target_idx - 1]
        ...
        return (
            item,
            ground_truth,
        )  # imgs, silhouette_imgs, body_joints, face_joints # , vertices
...
class dynamic_offsets_dataset(Dataset):
    ...
    def __getitem__(self, item):
        img_path = f"{self.root_dir}/frames_mat/{self.name}/{str(self.idx_list[item]).zfill(PADDING)}.png"
        ...
        silhouette_img_path = f"{self.root_dir}/mask_mat/{self.name}/{str(self.idx_list[item]).zfill(PADDING)}.png"
        ...
        naked_vertice_uv = np.load(
            f"{self.tmp_path}/naked_vertice_uv/{self.name}/{str(self.idx_list[item]).zfill(PADDING)}.npy"
        )
        ...
        if self.use_normal == True:
            ...
            normal = np.load(
                f"{self.root_dir}/normal/{self.name}/{str(self.idx_list[item]).zfill(PADDING)}_front.npz"
            )["arr_0"]
...
class neural_rendering_train_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        uv_map = np.load(f"{self.uv_dir}/{self.name}/{self.idx_list[idx]}.npz")[
            "arr_0"
        ]  #
        ...
        ref_img_mask = cv2.imread(
            f"{self.ref_dir}/{self.name}/mask_{self.idx_list[idx]}.png"
        )[
            :, :, :1
        ]  # / 255
...
class neural_rendering_test_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ref_img = cv2.imread(
            f"{self.ref_dir}/{self.name}/ref_{self.idx_list[idx]}.png"
        )  # '0001'
        ref_img = cv2.resize(ref_img, self.img_size)  # interpolation=cv2.INTER_AREA
        ref_img = cv2.cvtColor(ref_img, cv2.COLOR_BGR2RGB)
...
class neural_rendering_finetune_dataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        img = cv2.imread(
            f"{self.img_dir}/{self.name}/render_{self.idx_list[item]}.png"
        )  # self.img_list[item]
        ...
        uv_map = np.load(f"{self.uv_dir}/{self.name}/{self.idx_list[item]}.npz")[
            "arr_0"
        ]
