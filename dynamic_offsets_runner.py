...
class GeoTrainer:
    def __init__(
        ...
        tmp_path=None,
        load_path=None,
        base_path="./results/dynamic_offsets/",
    ):
        self.device = device
    ...
    def build_network(self, mode):
        self.network = dynamic_offsets_network(
            device=self.device,
            name=self.name,
    ...
    def build_dataloader(self, mode):
        dataset = dynamic_offsets_dataset(
    ...
    def loss(self, output, groundtruth):
        ...
        for i in range(output["v_shaped"].shape[0]):
            lap_loss += self.LapLoss(
                output["v_shaped"][i], output["v_shaped_personal"][i]
            )
        lap_loss = (
            lap_loss
            if self.isupsample == False
            else lap_loss / output["v_shaped"].shape[0]
        )
        ...
        if self.use_normal == True:
            ...
            gt_normal_imgs[
                torch.where(groundtruth["gt_sil_images"] == 0)
            ] = torch.from_numpy(np.zeros(3).astype(np.float32)).cuda()
        ...
        move_loss = torch.mean(
            torch.abs(output["v_shaped_personal"] - output["v_shaped"])
        )
    ...
    def forward(self, step, item, input, ground_truth):
        ...
        if step % 20 == 0:
            print(
                "step {}: l1_loss {} sil_loss {} lap_loss {} normal_loss {} edge_loss {} move_loss {}".format(
                    step,
                    loss_output["l1_loss"].item(),
                    loss_output["sil_loss"].item(),
                    loss_output["lap_loss"].item(),
                    loss_output["normal_loss"].item(),
                    loss_output["edge_loss"].item(),
                    loss_output["move_loss"].item(),
                )
            )
        if step % 80 == 0:
        ...
        for step, (item, input, ground_truth) in enumerate(self.dataloader):
            ...
            if write_obj == True:
                for i in range(v.shape[0]):
                    self.network.write_v_obj(
                        v=v[i],
                        path=self.results_path
                        + "/obj/{}.obj".format(str(_idx[i] + 1).zfill(PADDING)),
                    )
            if step == 0:
                self.network.write_v_obj(
                    v=v[0],
                    path=self.results_path
                    + "/obj/{}.obj".format(str(_idx[0] + 1).zfill(PADDING)),
                )
            for i in range(mi.shape[0]):
                img = mi[i, :, :, :3]
                cv2.imwrite(
                    self.results_path
                    + "/img/mesh_img_{}.png".format(str(_idx[i] + 1).zfill(PADDING)),
                    img * 255,
                )
                img = ri[i, :, :, :3]
                ...
                cv2.imwrite(
                    self.results_path
                    + "/img/render_img_{}.png".format(str(_idx[i] + 1).zfill(PADDING)),
                    img * 255,
                )
    ...
    def visualization(self, model_output, ground_truth):
        ...
        for i in range(a.shape[0]):
            ...
            self.network.write_v_obj(
                model_output["v"][i], f"{self.mid_results_path}v_{i}.obj"
            )
            self.network.write_v_obj(
                model_output["v_shaped_personal"][i],
        ...
        if self.use_normal == True:
            ...
            for i in range(m.shape[0]):
                ...
                normal = m[i, :, :, :3]
                normal = cv2.cvtColor(
                    normal, cv2.COLOR_RGB2BGR
                )  # sum = np.sum(np.square(normal), axis=2)
                normal_img = (normal * 0.5 + 0.5) * 255
                cv2.imwrite(
                    f"{self.mid_results_path}normal_img_{idx}_pred.png", normal_img
                )  # img * 255
                ...
                normal_img = (normal * 0.5 + 0.5) * 255
                cv2.imwrite(
                    f"{self.mid_results_path}normal_img_{idx}_gt.png", normal_img
                )  # img * 255

    def save_weights(self, epoch_idx=None):
        save_path = (
            self.checkpoints_path + f"do_network_{epoch_idx}.pt"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--root_dir", type=str, default="/mnt/8T/zh/vrc"
    )  # '/home/coder/vrc/'
    parser.add_argument(
        "--name", type=str, default="Body2D_2037_344"
    )  #  Body2D_2027_286  Body2D_2043_288  Body2D_2064_287 C0029 2040_01    Body2D_2070_380
    ...
    if "2" in process_flow:
        # target_path = './results'
        uv_pos_generator = UVPositionalMapGenerator(
            src_data_path=f"{tmp_path}/diff_optiming/{args.name}"
        )
        uv_pos_generator.generate_uv(
            target_uv_path=f"{tmp_path}/naked_vertice_uv/{args.name}"
        )
