...
class AposeEstimator:
    ...
    def build_network(self, mode):
        self.model = apose_estimator_network(
            self.device, norm="batch", pose_encoding="2"
        )
    ...
    def forward(self, item, model_input, ground_truth):
        model_output = self.model(
            model_input["masks"][0], model_input["body_joints"][0]
        )
        ...
        joints2d_loss = self.Joint2dLoss(
            model_output["perspective_body_joints_h"], ground_truth["body_joints"]
        )

        loss = 10 * joints2d_loss + 10 * pose_loss + 1 * trans_loss + 0 * betas_loss

        print(
            f"step {item} joints2d_loss:{joints2d_loss} pose_loss: {pose_loss} trans_loss:{trans_loss} betas_loss:{betas_loss}"
        )
        ...
        for epoch_idx in range(outer_epoch):
            ...
            for name in name_list:
                ...
                for _ in range(inner_epoch):
                    for step, (item, model_input, ground_truth) in enumerate(
                        self.dataloader
                    ):
                        ...
                        pose = (
                            rodrigues(pose.reshape([-1, 1, 3]))
                            .reshape([-1, 24, 3, 3])
                            .unsqueeze(0)
                        )  # axisangle to rotation matrix
    ...
    def test(self, name, save_path):
        ...
        with torch.no_grad():
            for step, (item, model_input) in enumerate(self.dataloader):
                ...
                pose_pred = np.array(
                    [
                        cv2.Rodrigues(pose_pred_matrix[i])[0]
                        for i in range(pose_pred_matrix.shape[0])
                    ]
                ).squeeze()
                ...
                trans_pred_save[
                    step : step + self.view_num * self.num : self.num
                ] = tensor2numpy(model_output["trans"][0])
                betas_pred_save = tensor2numpy(model_output["betas"][0])

                # just for visualization
                if False:
                    for idx in range(model_output["v"].shape[1]):
                        # self.model.smpl.smpl.write_obj(v[0, idx], save_path + f'{str(step + idx * self.num).zfill(PADDING)}.obj')
                        _pad = torch.zeros_like(
                            model_input["body_joints"][0, idx, :, 2:]
                        )
                        self.model.smpl.smpl.write_obj(
                            torch.cat(
                                [model_input["body_joints"][0, idx, :, :2], _pad], dim=1
                            ),
                            save_path + f"joints_{step + idx * self.num}_gt.obj",
                            write_face=False,
                        )
                        self.model.smpl.smpl.write_obj(
                            torch.cat(
                                [
                                    model_output["perspective_body_joints_h"][
                                        0, idx, :, :2
                                    ],
                                    _pad,
                                ],
                            ...
                            save_path + f"joints_{step + idx * self.num}_pred.obj",
                            write_face=False,
                        )
