...
def select_keyframes(json_dir):
    ...
    for j_idx in range((int)(len(joints_list) / 8) * 8):
        body_joint, face_joint = load_joints_from_openpose(
            f"{json_dir}{joints_list[j_idx]}"
        )  # {root_dir}2d_joints/{name}/json/
    ...
    front_hand_distance_y[np.where(front_or_back == 1)] = hand_distance_y[
        np.where(front_or_back == 1)
    ]
    back_hand_distance_y = np.ones_like(hand_distance_y) * np.Inf
    back_hand_distance_y[np.where(front_or_back == 0)] = hand_distance_y[
        np.where(front_or_back == 0)
    ]
...
class transformation_flow:
    def __init__(self, device, vertices=None, faces=None, textures=None, img_size=1080):
        ...
        K = np.tile(
            np.array([[f, 0, f / 2], [0, f, f / 2], [0, 0, 1]]), [batch_size, 1, 1]
        )

        self.renderer = nr.Renderer(
            camera_mode="projection", R=R, t=t, K=K, image_size=self.img_size
        )
        ...
        self.fs = (
            torch.from_numpy(np.expand_dims(f, axis=0))
            .repeat([batch_size, 1, 1])
            .type(torch.long)
            .cuda()
        )  # .to(device)
        self.fts = (
            torch.from_numpy(np.expand_dims(ft, axis=0))
            .repeat([batch_size, 1, 1])
            .type(torch.long)
            .cuda()
        )  # .to(device)
        self.vts = (
            torch.from_numpy(np.expand_dims(vt, axis=0))
            .repeat([batch_size, 1, 1])
            .type(torch.float)
            .cuda()
        )  # .to(device)
        ...
        self.textures = torch.ones(
            1, 13776, texture_size, texture_size, texture_size, 3, dtype=torch.float32
        ).cuda()
        self.device = device

    def cal_bc_transform(
        self, from_faces_verts_on_img, dst_fims, dst_wims, visibility, image_size
    ):
        """
        Args:
        ...
        T = -1 * torch.ones(
            (bs, image_size * image_size, 2), dtype=torch.float32, device=self.device
        )
        mask = torch.zeros(
            (bs, image_size * image_size, 1), dtype=torch.float32, device=self.device
        )
        ...
        for i in range(bs):
            ...
            exist_smpl_T = (
                from_faces_verts_on_img[0][to_exist_face_idx]
                * to_exist_face_weights[:, :, None]
            ).sum(dim=1)
            # (256, 256, 2)
            T[i, to_exist_mask] = exist_smpl_T
            mask[i, to_exist_mask] = visibility[
                to_exist_face_idx
            ]  # torch.ones([exist_smpl_T.shape[0], 1]).cuda()
    ...
    def load_src_vertices_list(self, src_vertices_list):
        ...
        for idx in range(len(src_vertices_list)):
            src_vertices = src_vertices_list[
                idx
            ]  # torch.from_numpy(np.expand_dims(src_vertices_list[idx], axis=0)).cuda()

            src_model_images, _, _ = self.renderer(
                src_vertices, self.faces, self.textures
            )

            src_f2verts, src_fim, src_wim = self.renderer(
                src_vertices, self.faces, mode="fim_and_wim"
            )
            ...
            # cal area
            ab = (
                from_faces_verts_on_img[:, 1, :] - from_faces_verts_on_img[:, 0, :]
            ) * 10
            ac = (
                from_faces_verts_on_img[:, 2, :] - from_faces_verts_on_img[:, 0, :]
            ) * 10
            ...
            src_face_index = torch.unique(
                eroded_face_index_map
            )  # torch.unique(to_src_face_index_map)
            # -------------------------------
            ...
            from_faces_verts_on_img[invis_src_face_index] = (
                -1 * torch.ones([3, 2]).cuda()
            )
    ...
    def warp(self, src_img, src_idx):
        ...
        tar_model_images, _, _ = self.renderer(
            self.tar_vertices, self.faces, self.textures
        )
        tar_f2verts, tar_fim, tar_wim = self.renderer(
            self.tar_vertices, self.faces, mode="fim_and_wim"
        )
        T, mask = self.cal_bc_transform(
            self.src_p2verts_list[src_idx],
            tar_fim,
            tar_wim,
            self.visibility_list[src_idx],
            self.img_size,
        )
    ...
    def forward(self, src_img):
        src_model_images, _, _ = self.renderer(
            self.src_vertices, self.faces, self.textures
        )
...
class UVMapGenerator:
    def __init__(self, device, root_dir, src_data_path, name):
        ...
        # self.num = num - num % 8
        model_path = (
            "./assets/neutral_smpl.pkl"
            if self.isupsample == False
            else "./assets/upsample_neutral_smpl.pkl"
        )
        self.smpl = SMPLModel(
            device=self.device, model_path=model_path, use_posematrix=False
        )
        ...
        self.offset = numpy2tensor(np.load(src_data_path + "/offsets.npy")).to(
            self.device
        )
    ...
    def generate_uv(
        ...
        for pose_idx in range(self.pose.shape[0]):
            vert = self.smpl(
                pose=self.pose[pose_idx : pose_idx + 1],
                betas=self.betas[0:1],
                trans=self.trans[pose_idx : pose_idx + 1],
                displacements=self.offset[pose_idx : pose_idx + 1],
            )
        ...
        if pose_seq == "src":
            ...
            for pose_idx in range(360):
                pose_c[0] = pose_c[0] + torch.Tensor([0, 1 / 360 * 3.14 * 2, 0]).to(
                    self.device
                )
                vert = self.smpl(
                    pose=pose_c.reshape([-1]),
                    betas=self.betas[0:1],
                    trans=self.trans[0:1],
                    displacements=self.offset[0:1],
                )
                verts.append(vert)
        ...
        if generate_ref == True:
            ...
            for keyframe in keyframes:
                src_img = cv2.imread(
                    f"{self.root_dir}/frames_mat/{self.name}/{str(keyframe).zfill(PADDING)}.png"
                )
                src_img = cv2.resize(src_img, (img_size, img_size))
                src_img = np.transpose(
                    cv2.cvtColor(src_img, cv2.COLOR_BGR2RGB), [2, 0, 1]
                )
        ...
        with torch.no_grad():
            for pose_idx in tqdm.tqdm(range(len(verts))):
                ...
                uvs = self.render.get_uv_img(
                    verts[pose_idx], output_path="vis_uv"
                )  # .squeeze()
                uv = uvs[0]
                np.savez_compressed(
                    target_uv_path + "/" + str(pose_idx + 1).zfill(PADDING), uv
                )
                ...
                if generate_ref == True:
                    ...
                    for src_idx in range(len(src_img_list)):
                        ...
                        image = tensor2numpy(tar_img)[0].transpose((1, 2, 0))[
                            :, :, ::-1
                        ]
                        mask_img = tensor2numpy(mask)[0]

                        mask_img[np.where(full_mask[:, :, 0] == 1)] = 0
                        final_img[np.where(mask_img[:, :, 0] == 1)] = image[
                            np.where(mask_img[:, :, 0] == 1)
                        ]
                        full_mask[np.where(mask_img[:, :, 0] == 1)] = 1

                    cv2.imwrite(
                        f"{target_ref_path}/mask_{str(pose_idx+1).zfill(PADDING)}.png",
                        full_mask * 255,
                    )
                    cv2.imwrite(
                        f"{target_ref_path}/ref_{str(pose_idx+1).zfill(PADDING)}.png",
                        final_img * 255,
                    )
...
class UVPositionalMapGenerator:
    def __init__(self, src_data_path):
        ...
        self.smpl = SMPLModelNUMPY(
            model_path="./assets/neutral_smpl.pkl", use_posematrix=False
        )
    ...
    def generate_uv(self, target_uv_path):
        ...
        for idx in tqdm.tqdm(range(self.pose.shape[0])):
            naked_vertice = self.smpl.set_params(
                beta=self.betas[0],
                pose=self.pose[idx],
            ...
            for face_idx in range(self.fs.shape[0]):
                ...
                lengtha = sqrt(
                    (posax - posbx) * (posax - posbx)
                    + (posay - posby) * (posay - posby)
                )
                lengthb = sqrt(
                    (posbx - poscx) * (posbx - poscx)
                    + (posby - poscy) * (posby - poscy)
                )
                lengthc = sqrt(
                    (poscx - posax) * (poscx - posax)
                    + (poscy - posay) * (poscy - posay)
                )
            ...
            np.save(
                f"{target_uv_path}/{str(idx+1).zfill(PADDING)}.npy", uv_positional_map
            )
            cv2.imwrite(
                f"{target_uv_path}/{str(idx+1).zfill(PADDING)}.png",
                (uv_positional_map / 2 + 0.5) * 255,
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--name", type=str, default="Body2D_2040_499"
    )  #  Body2D_2027_286 Body2D_2010_378 Body2D_2061_507 Body2D_2040_499   Body2D_2041_308 Body2D_2070_380
    ...
    uv_generator = UVMapGenerator(
        device=device, src_data_path=f"./results/dynamic_offsets/{name}", name=name
    )
    uv_generator.generate_uv(
        target_uv_path=f"{target_path}/uvs/{name}",
    ...
    uv_generator.generate_uv(
        target_uv_path=f"{target_path}/test_uvs/{name}",
        target_ref_path=f"{target_path}/test_ref/{name}",
        pose_seq="self_rotate",
        img_size=1024,
        generate_ref=True,
    )
    uv_generator.generate_uv(
        target_uv_path=f"{target_path}/double_test_uvs/{name}",
        pose_seq="self_rotate",
        img_size=2048,
        generate_ref=False,
    )
